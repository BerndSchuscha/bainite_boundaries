...
class ModelPipeline:
    def __init__(self, model, model_name, input_transform=True, output_transform=None):
        ...
        # robust scaler
        self.scaler = RobustScaler() if input_transform else None
    ...
    def get_interval_predictions(self, x, coverage, distance=None):
        ...
        if self.task_type == 'classification': 
            ...
            confidence_sets_distance = (y_pred_proba >= 1 - self.CP_quantiles_distance).astype(int)
    ...
    def compute_errors(self, y_true, prediction_dict):
        ...
        if self.task_type == 'classification':
            ...
            if y_true is None:
                ...
                interval_size = np.mean( (prediction_dict['upper_CP'] - prediction_dict['lower_CP']) / y_true )
                ...
                interval_size_distance = np.mean( ( prediction_dict['upper_CP_distance'] - prediction_dict['lower_CP_distance'] ) / y_true )
...
def run_cross_validation(x, y, 
    ...
    if n_splits == 1:
        kf = KFold(n_splits=n_splits+5, shuffle=True)
    else:
        kf = KFold(n_splits=n_splits, shuffle=True)
        
    if n_splits_CP == 1:
        kf_CP = KFold(n_splits=n_splits_CP+1, shuffle=True)
    else:
        kf_CP = KFold(n_splits=n_splits_CP, shuffle=True)
    results = []
    for i, (train_index_out, val_index_out) in enumerate(kf.split(x)):

        logger.info(f'Validation fold (outer) {i+1}/{n_splits}')
        
        
        for j, (train_index_CP, val_index_CP) in enumerate(kf_CP.split(train_index_out)):
